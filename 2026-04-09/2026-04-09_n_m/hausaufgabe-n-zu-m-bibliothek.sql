...
-- //der index für den lesernamen ist sinnvoll um die Person die gerade etwas ausleihen oder zurück geben will schnell zu finden.
-- //der index für den Buchtitel ist sinnvoll um schnell zu sehen ob das Buch in der Bibliothek vorhanden ist oder nicht.
-- //der index für die Abfrage der aktuell ausgeliehenen Bücher ist sinnvoll um schnell zu sehen welche Bücher gerade ausgeliehen sind.
-- DEIN CODE HIER:
create INDEX idx_leser_name ON leser(name);
create INDEX idx_buch_titel ON buch(titel);
create INDEX idx_ausleihe ON ausleihe(rueckgabe_datum);
...
--
-- DEIN CODE HIER:
Select l.name, b.titel, a.ausleih_datum, a.rueckgabe_datum from ausleihe a join leser l on a.leser_id = l.id join buch b on a.buch_id = b.id order by a.ausleih_datum desc ;

Select buch.titel, leser.name, ausleihe.ausleih_datum from buch join ausleihe on buch.id = ausleihe.buch_id join leser on ausleihe.leser_id = leser.id where ausleihe.rueckgabe_datum is null;

Select buch.titel, count(ausleihe.buch_id) from buch join ausleihe on buch.id=ausleihe.buch_id group by buch.titel;

Select leser.name, count(ausleihe.leser_id) as anzahl_ausleihen from leser join ausleihe on leser.id = ausleihe.leser_id group by leser.name order by anzahl_ausleihen desc limit 1;
...
--
-- DEIN CODE HIER:
    Create View aktuelle_ausleihen as Select buch.titel, leser.name, ausleihe.ausleih_datum from buch join ausleihe on buch.id = ausleihe.buch_id join leser on ausleihe.leser_id = leser.id where ausleihe.rueckgabe_datum is null;
